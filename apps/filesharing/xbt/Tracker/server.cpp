...
static void* p4p_update_threadfunc(void* arg)
{
	p4p::app::P4PUpdateManager* update_mgr = (p4p::app::P4PUpdateManager*)arg;
	update_mgr->run();
	return NULL;
}

static std::string extract_hostname(const std::string& addr)
{
	std::string::size_type sep = addr.find(':');
	if (sep == std::string::npos)
		return "";
	return addr.substr(0, sep);
}

static unsigned short extract_port(const std::string& addr)
{
	std::string::size_type sep = addr.find(':');
	if (sep == std::string::npos)
		return 0;
	return (unsigned short)atoi(addr.substr(sep + 1).c_str());
}
...
static std::vector<std::string> split_string(const std::string& s, char delim)
{
	...
	do
	{
		std::string::size_type cur = s.find(delim, prev);
		std::string token;
...
int Cserver::run()
{
	...
	if (m_config.m_debug)
		p4p::setLogLevel(p4p::LOG_DEBUG);

	BOOST_FOREACH(std::string portal, split_string(m_config.m_portal_addresses, ','))
	{
		std::string host = extract_hostname(portal);
		unsigned short port = extract_port(portal);

		if (host.empty() || port == 0)
			continue;

		isp_mgr.addISP(portal, host, port);
	}

	update_mgr = new p4p::app::P4PUpdateManager(isp_mgr);
	...
	if (pthread_create(&update_mgr_thd, NULL, p4p_update_threadfunc, update_mgr) != 0)
	{
		std::cerr << "Failed to spawn P4P Update Manager thread" << std::endl;
		exit(1);
	}
...
std::string Cserver::insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	if (file.selection_mgr == NULL)
		file.init_p4p(&isp_mgr, update_mgr,
				extract_hostname(m_config.m_aoe_address),
				extract_port(m_config.m_aoe_address));
	if (!file.ctime)
		file.ctime = time();
	...
	if (v.m_event == Ctracker_input::e_stopped)
	{
		...
		{
			file.selection_mgr->removePeer(&itr->second);
			file.peers.erase(itr);
		}
	}
	else
	{
		t_peers::iterator peer_itr = file.peers.find(peer_key);
		if (peer_itr == file.peers.end())
		{
			peer_itr = file.peers.insert(std::make_pair(peer_key, t_peer(v.m_ipa))).first;
			file.selection_mgr->addPeer(&peer_itr->second);
		}
		t_peer& peer = peer_itr->second;
		...
		peer.mtime = time();
		file.selection_mgr->updatePeerStats(&peer);
	}
...
std::string Cserver::t_file::select_peers(const Ctracker_input& ti) const
{
	 ...
	 * NOTE: Doesn't handle password-protected torrents since we
	 *       don't have direct access to the user ID here, which
	 *       is needed to properly construct the peer key.
	 */
	t_peers::const_iterator peer_itr = peers.find(peer_key_c(ti.m_ipa, 0));
	if (peer_itr == peers.end())
		return "";

	size_t c = ti.m_num_want < 0 ? 50 : std::min(ti.m_num_want, 50);
	std::vector<const p4p::app::P4PPeerBase*> p4p_selected_peers;
	p4p_selected_peers.reserve(c);
	if (selection_mgr->selectPeers(&peer_itr->second, c, p4p_selected_peers) == 0)
	{
		std::string d;
		d.resize(6 * p4p_selected_peers.size());
		for (unsigned int i = 0; i < p4p_selected_peers.size(); ++i)
		{
			t_peer* peer = (t_peer*)p4p_selected_peers[i];
			memcpy(&d[6 * i], &peer->ipaddr, 4);
			memcpy(&d[6 * i] + 4, &peer->port, 2);
		}
		return d;
	}
	...
	else if (!m_config.m_auto_register)
	{
		...
		while (getline(is, s))
		{
			...
			if (s.size() != 20)
				continue;
			m_files[s];
			if (m_files[s].selection_mgr == NULL)
				m_files[s].init_p4p(&isp_mgr, update_mgr,
							extract_hostname(m_config.m_aoe_address),
							extract_port(m_config.m_aoe_address));
			new_files.insert(s);
		}
