...
#include "p4p_config.h"		/* For parsing P4P configuration */

using namespace p4p;

static volatile bool g_sig_term = false;

void shuffle_peerlist(std::vector<const P4PPeerBase*> *pl)
{
	std::vector<const P4PPeerBase*>::iterator start, end;
	start = pl->begin();
	end = pl->end();
...
void Cserver::add_isp(std::string loc_ip, int loc_port, std::string dis_ip, int dis_port) {
	ISP *new_isp = new ISP(loc_ip, loc_port, dis_ip, dis_port);
	new_isp->update();
	m_isp_list.push_back(new_isp);
}
	...
	m_database(database)
{
	...
	m_time = ::time(NULL);
	m_use_sql = use_sql;
	m_guidance_server_ip = "NA";
	m_guidance_server_port = 0;
	/* Read P4P configuration including ISPs and guidance server address */
	read_p4p_config("isp_conf.xml", this);
	if (m_guidance_server_ip!="NA") {
		for (unsigned int i = 0; i < m_isp_list.size(); i++) {
			ISPGenericPeeringGuidance *aoe_guidance = new ISPGenericPeeringGuidance(m_isp_list[i], m_guidance_server_ip, m_guidance_server_port);
			aoe_guidance->update();
			m_guidance_map.insert(std::map<ISP*, ISPGenericPeeringGuidance*>::value_type(m_isp_list[i], aoe_guidance));
		}
	}
}
...
int Cserver::run()
{
	...
	/*NOT supported yet: Start P4P background threads */
	//m_p4p_threads.create_thread(boost::bind(&Cserver::update_isps, this));
	//m_p4p_threads.create_thread(boost::bind(&Cserver::update_guidance, this));
	...
	//m_p4p_threads.interrupt_all();
	write_db_files();
	write_db_users();
	//m_p4p_threads.join_all();
...
std::string Cserver::insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	t_file& file = m_files.insert(std::make_pair(v.m_info_hash, t_file(*this))).first->second;	
	...
	{
		t_peers::iterator itr = file.peers.find(peer_key);
		if (itr != file.peers.end())
		{
			std::map<P4PPeerBase*, t_peer*>::iterator it = file.peer_map.find(&itr->second.p4p_peer);
			if (it != file.peer_map.end()) {
				file.peer_map.erase(it);
			}
			file.isp_view_manager->removePeer(&itr->second.p4p_peer);
			file.peers.erase(itr);
		}
	}
	else
	{
		t_peer& peer = file.peers.insert(std::make_pair(peer_key, t_peer(v.m_ipa, v.m_port))).first->second;
		
		/* add peer's P4P hook */
		file.isp_view_manager->addPeer(&peer.p4p_peer, IPAddress(AF_INET, (void *)&v.m_ipa));
		std::map<P4PPeerBase*, t_peer*>::iterator it = file.peer_map.find(&(peer.p4p_peer));
		if (it != file.peer_map.end()) {
			file.peer_map[&(peer.p4p_peer)] = &peer;
		}
		else {
			file.peer_map.insert(std::map<P4PPeerBase*, t_peer*>::value_type(&(peer.p4p_peer), &peer));
		}
	...
	/* Utility function: select peers from PIDs given weights */
	void select_peers_from_pids(
			std::map<const PID*, P4PPeerList*>& pid_peers,
			std::map<const PID*, double>& pid_probs,
			unsigned int max_peers,
			P4PPeerList& out_peers)
	{
		/* Skip if no PIDs */
		if (pid_peers.empty())
			return;

		/* Skip if no match between pid_peers and pid_probs ? 
		if (pid_peers.size() != pid_probs.size())
			return;
		*/

		/* Compute totals */
		unsigned int total = 0;
		for(std::map<const PID*, P4PPeerList*>::const_iterator itr = pid_peers.begin(); itr != pid_peers.end(); ++itr)
			total += itr->second->size();

		double cummulative = 0.0;
		for(std::map<const PID*, double>::iterator itr = pid_probs.begin(); itr != pid_probs.end(); ++itr)
			cummulative += itr->second;

		do
		{
			/* Choose a random bin and determine which bin r falls into */
			double r = (rand() / (double)RAND_MAX) * cummulative;
			std::map<const PID*, double>::iterator probs_itr = pid_probs.begin(); 
			while(r >= probs_itr->second)
			{
				r -= probs_itr->second;
			       	++probs_itr;
			}
			...
			const PID* pid = probs_itr->first;
			std::map<const PID*, P4PPeerList*>::iterator peers_itr = pid_peers.find(pid);
			P4PPeerList *pidpeerlist;
			if( (peers_itr != pid_peers.end()) && ((pidpeerlist = peers_itr->second) != NULL) && (pidpeerlist->size() > 0))
			{
				P4PPeerList::iterator p_itr = pidpeerlist->begin();
				p_itr += rand() % pidpeerlist->size();
				const P4PPeerBase* peer = *p_itr;
				if (!peer) throw std::runtime_error("illegal state: expected to retrieve non-null peer");

				out_peers.push_back(peer);

				pidpeerlist->erase(p_itr);
				total -= 1;
			}
			else 
			{
				cummulative -= probs_itr->second;
				probs_itr->second = 0;
			}
		} while (out_peers.size() < max_peers && total > 0);
	}
...
std::string Cserver::t_file::select_peers(const Ctracker_input& ti) const
{
	...
	/* Determine peer making the selection
	 * TODO: Doesn't handle password protected torrents since
	 *       we don't have direct access to the user ID here, which
	 *       is needed to properly construct the peer_key. */
		
	t_peers::const_iterator sel_peer = peers.find(peer_key_c(ti.m_ipa, 0));
	if (sel_peer == peers.end())
		return "";
	
	/* Compute number of peers wanted */
	size_t c = ti.m_num_want < 0 ? 50 : std::min(ti.m_num_want, 50);
	P4PPeerList p4p_peers;
	const P4PPeerBase sel_p4p_peer = sel_peer->second.p4p_peer;
	const ISP *homeISP = sel_p4p_peer.getHomeISP();

	if (sel_p4p_peer.getHomeISP() == NULL) {
		/* use default (random) selection if homeless */
		int peer_num = c;
		if (peer_num >= peers.size()) {
			peer_num = peers.size();
		}
		for (t_peers::const_iterator peer_it = peers.begin(); (peer_it != peers.end()) && (peer_num > 0); peer_it++) {
			P4PPeerBase peer = peer_it->second.p4p_peer;
			p4p_peers.push_back(&peer);
			peer_num--;
		}
	}
	else {
		ISPView *home_isp_view = isp_view_manager->getISPView(homeISP);
		const PID homePID = sel_p4p_peer.getHomePID();
		
		std::map<const ISP*, ISPGenericPeeringGuidance*>::const_iterator guidance_it = server_ref->m_guidance_map.find(homeISP);
		if (guidance_it == server_ref->m_guidance_map.end()) {
			/* no guidance for home ISP; use default (random?) selection */
			int peer_num = c;
			if (peer_num >= peers.size()) {
				peer_num = peers.size();
			}
			...
			/* Retrieve guidance for home ISP PID */
			ISPGenericPeeringGuidance *aoe_guidance = guidance_it->second;

			/* Select intra-PID peers */
			double intrapid_up_percent = aoe_guidance->getIntraPIDPct(homePID);
			
			P4PPeerList intra_pid_peers;
			home_isp_view->listPeers(homePID, intra_pid_peers);
			int intra_pid_num = int(intrapid_up_percent * c);

			//TODO: to be more efficient  
			//for(P4PPeerList::iterator itr = intra_pid_peers.begin(); itr < 
			if (intra_pid_num >= intra_pid_peers.size()) {
				intra_pid_num = intra_pid_peers.size();
			}
			else {
				shuffle_peerlist(&intra_pid_peers);
			}
			p4p_peers.insert(p4p_peers.end(), intra_pid_peers[0], intra_pid_peers[intra_pid_num]);
			
			/* Select intradomain peers */
			double intradomain_up_percent = aoe_guidance->getIntradomainPct(homePID);
			unsigned int intradomain_num = int(c * intradomain_up_percent) - p4p_peers.size();

			PIDList internal_pids;
			homeISP->listInternalPIDs(internal_pids);
			std::map<const PID*, P4PPeerList&> intradomain_peers;
			std::map<const PID*, double> intradomain_probs;

			for(PIDList::iterator pid_itr = internal_pids.begin(); pid_itr != internal_pids.end(); ++pid_itr) {
				if (homePID == *pid_itr) {
					continue;
				}
				double weight = aoe_guidance->getWeight(homePID, *pid_itr);
				P4PPeerList pidpeerlist;
				home_isp_view->listPeers(*pid_itr, pidpeerlist);
				intradomain_peers.insert(std::make_pair(*pid_itr, &pidpeerlist));
				intradomain_probs.insert(std::make_pair(*pid_itr, weight));
			}
			select_peers_from_pids(intradomain_peers, intradomain_probs, intradomain_num, p4p_peers);
			
			/* Select interdomain peers */
			int interdomain_num = c - p4p_peers.size();

			PIDList external_pids;
			homeISP->listExternalPIDs(external_pids);
			std::map<const PID*, P4PPeerList&> interdomain_peers;
			std::map<const PID*, double> interdomain_probs;

			for(PIDList::iterator pid_itr = external_pids.begin(); pid_itr != external_pids.end(); ++pid_itr) {
				double weight = aoe_guidance->getWeight(homePID, *pid_itr);
				P4PPeerList pidpeerlist;
				home_isp_view->listPeers(*pid_itr, pidpeerlist);
				interdomain_peers.insert(std::make_pair(*pid_itr, &pidpeerlist));
				interdomain_probs.insert(std::make_pair(*pid_itr, weight));
			}
			select_peers_from_pids(interdomain_peers, interdomain_probs, interdomain_num, p4p_peers);

			/* Padding with random peers ?
			if(p4p_peers.size() < c)
			{
			}
			*/
	...
	/* Encode output string for the selected peers */
	std::string d;
	d.reserve(300);
	for (int i = 0; i < p4p_peers.size(); i++)
	{
		if (peer_map.find(p4p_peers[i]) != peer_map.end()) {
			boost::array<char, 6> v;
			memcpy(&v.front(), peer_map[p4p_peers[i]]->ipa, 4);
			unsigned short port = peer_map[p4p_peers[i]]->port;
			memcpy(&v.front() + 4, &port, 2);
...
void Cserver::t_file::clean_up(time_t t, Cserver& server)
{
	...
	for (int i = 0; i < server_ref->m_isp_list.size(); i++) {
		isp_view_manager->removeISPView(server_ref->m_isp_list[i]);
	}
	delete isp_view_manager;
}
...
void Cserver::clean_up()
{
	...
	std::map<ISP*, ISPGenericPeeringGuidance*>::iterator it;
	for (it = m_guidance_map.begin(); it != m_guidance_map.end(); it++) {
		delete it->second;
	}
	
	for (int i = 0; i < m_isp_list.size(); i++) {
		delete m_isp_list[i];
	}
	m_guidance_map.clear();
	m_isp_list.clear();
...
void Cserver::read_db_files()
{
	...
	else if (!m_config.m_auto_register)
	{
		...
		while (getline(is, s))
		{
			...
			t_file& file = m_files.insert(std::make_pair(s, t_file(*this))).first->second;
			new_files.insert(s);
		}
	...
	try
	{
		...
		for (Csql_row row; row = result.fetch_row(); )
		{
			...
			if (row[0].size() != 20 || file(row[0].s()))
				continue;
			t_file& file = m_files.insert(std::make_pair(row[0].s(), t_file(*this))).first->second;
			if (file.fid)
				continue;
...
/*TODO:
void Cserver::update_isps(Cserver* server)
{
	;
}

void Cserver::update_guidance(Cserver* server)
{
	;
}
*/
